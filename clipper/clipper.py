...
# https://github.com/Geri-Borbas/Clipper                                       #
# ===============================================================================
...
def _FindSegment(outPt, pt1, pt2):
    ...
    outPt2 = outPt
    while True:
        if _SlopesEqual(pt1a, pt2a, outPt.pt, outPt.prevOp.pt) and _SlopesEqual(pt1a, pt2a,
                                                                                outPt.pt):
    ...
    def _InsertLocalMinimaIntoAEL(self, botY):
        while self._CurrentLocMin is not None and \
            ...
            if rb.outIdx >= 0 and rb.dx == horizontal and self._HorzJoins is not None:
                hj = self._HorzJoins
                while True:
                    dummy1, dummy2, overlap = _GetOverlapSegment(hj.edge.Bot, hj.edge.Top, rb.Bot,
                                                                 rb.Top)
                    if overlap:
                    ...
                    if e.outIdx >= 0:
                        ...
                        if hj is not None:
                            while True:
                                ...
                                if overlap: self._AddJoin(hj.edge, e.nextInLML, hj.savedIdx,
                                                          e.outIdx)
                                hj = hj.nextHj
                            if hj == self._HorzJoins: break
...
def _OffsetInternal(polys, isPolygon, delta, jointype=JoinType.Square, endtype=EndType.Square,
                    limit=0.0):
    def _DoSquare(pt):
