...
class Button(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.x <= x <= self.x + self.w and self.y <= y <= self.y + self.h:
            pygame.draw.rect(self.screen, self.ONtextcolor, (int(self.x - self.h * 0.05), int(self.y - self.h * 0.05),
                                                        int(self.w + self.h * 0.1), int(self.h + self.h * 0.1)), 1)
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        print(self.signal, self.signaltype)
        else:
            pygame.draw.rect(self.screen, self.ONtextcolor, (self.x - 2, self.y - 2,
                                                             self.w + 4, self.h + 4), 1)
        self.screen.blit(self.UNDERrendertext, (self.UNDERx, self.UNDERy))
...
for i in range(300, 750, 250):
    for j in range(120, 880, 130):
        counting += 1
        button = Button(j, i, 100, 100, screen, 1, 1, imagepath="data", imagename="иконка228.png", ONtext=f"{counting}",
